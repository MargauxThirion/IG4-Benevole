...
exports.createAccueil = (req, res, next) => {
  ...
  accueil
    .save()
    .then(() => {
      res.status(201).json({ message: "Accueil créé !" });
    })
    .catch((error) => {
...
exports.modifyAccueil = (req, res, next) => {
  ...
  Accueil.updateOne({ _id: req.params.id }, accueil)
    .then(() => {
      res.status(201).json({ message: "Accueil modifié !" });
    })
    .catch((error) => {
      res.status(400).json({ error: error });
    });
...
exports.deleteAccueil = (req, res, next) => {
  Accueil.deleteOne({ _id: req.params.id })
    .then(() => {
      res.status(200).json({ message: "Accueil supprimé !" });
    })
    .catch((error) => {
      res.status(400).json({ error: error });
    });

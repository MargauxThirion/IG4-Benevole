...
exports.getJeuByNom = async (req, res, next) => {
  ...
  try {
      ...
      const jeu = await Jeu.findOne({ nom_jeu: { $regex: new RegExp('^' + gameName + '$', 'i') } });
      ...
      const zone = await ZoneBenevole.findOne({ liste_jeux: jeu._id }).populate('nom_zone_benevole');
...
exports.searchJeux = async (req, res) => {
  ...
  try {
      const jeux = await Jeu.find({ nom_jeu: { $regex: '^' + searchTerm, $options: 'i' } }).limit(10);

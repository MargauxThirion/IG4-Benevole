    ...
    .then((flexible) => {
      if (flexible) {
        res
          .status(200)
          .json({
            message: "Flexible créé ou mis à jour avec succès !",
            data: flexible,
          });
      } else {
        res
          ...
          .json({
            message:
              "Flexible non trouvé et non créé pour une raison inconnue.",
          });
      }
    })
    .catch((error) => {
...
exports.removeAllFlexible = (req, res, next) => {
  Flexible.deleteMany()
    .then(() => {
      res.status(200).json({ message: "Flexible supprimés !" });
    })
    .catch((error) => {
      res.status(400).json({ error: error });
    });
...
exports.removeOneFlexibleById = async (req, res) => {
  try {
    ...
    if (result.modifiedCount === 0) {
      return res
        .status(404)
        .json({ message: "Flexible non trouvé ou date non correspondante" });
    }

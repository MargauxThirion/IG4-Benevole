...
exports.createAnimation = (req, res, next) => {
    ...
    animation.save()
    .then(() => {res.status(201).json({message: 'Animation créée !'})})
...
exports.modifyAnimation = (req, res, next) => {
    ...
    .then(() => {res.status(201).json({message: 'Animation modifiée !'})})
    .catch((error) => {res.status(400).json({error: error})})
};

exports.deleteAnimation = (req, res, next) => {
    Animation.deleteOne({_id: req.params.id})
    .then(() => {res.status(200).json({message: 'Animation supprimée !'})})
    .catch((error) => {res.status(400).json({error: error})})
};

exports.getAllAnimation = (req, res, next) => {
    Animation.find()
    .then((animations) => {res.status(200).json(animations)})
    .catch((error) => {res.status(400).json({error: error})})
};
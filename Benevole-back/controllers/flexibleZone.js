...
exports.createFlexibleZone = (req, res, next) => {
    ...
    .then(flexibleZone => {
        if (flexibleZone) {
            res.status(200).json({ message: 'FlexibleZone créé ou mis à jour avec succès !', data: flexibleZone });
        } else {
            res.status(404).json({ message: 'FlexibleZone non trouvé et non créé pour une raison inconnue.' });
        }
    })
    .catch(error => {
...
exports.getAllFlexibleZone = (req, res, next) => {
    ...
    .populate('benevole_id', 'pseudo')
    .populate('horaire.liste_zoneBenevole', 'nom_zone_benevole')
    .then((flexibleZone) => {res.status(200).json(flexibleZone)})
    .catch((error) => {res.status(400).json({error: error})})
};

exports.removeAllFlexibleZone = (req, res, next) => {
    FlexibleZone.deleteMany()
    .then(() => {res.status(200).json({message: 'FlexibleZone supprimés !'})})
    .catch((error) => {res.status(400).json({error: error})})
};
...
exports.removeOneFlexibleZoneById = async (req, res) => {
    try {
        ...
        if (result.modifiedCount === 0) {
            return res.status(404).json({ message: "FlexibleZone non trouvé ou date non correspondante" });
        }
...
exports.checkAndDeleteFlexibleZone = async (req, res) => {
    try {
      ...
      res.status(200).json({ message: 'Opération réussie', deletedFlexibleZones });
    } catch (error) {
